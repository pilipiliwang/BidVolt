...
典型流程：登录 → 创建/进入项目 → 选择投标任务类型 → 准备企业资料与项目材料 → 确认材料 → Agent 编制/审核 → 查看成果、补充要求、评审与报价。

| 模块 | 当前前端能力 | 对接边界 |
| --- | --- | --- |
| 账号与项目 | 登录、注册、会话刷新；项目搜索、新增、归档和阶段入口 | 使用真实后端；任务类型入口记忆仍依赖浏览器与 URL，不代表跨设备同步 |
| 企业资料库 | 分类树、上传/预览、字段纠正及版本记录；各阶段统一资料展示 | 企业长期资料与当前项目招标材料分开管理 |
| 上传材料 | 招标/补充材料上传、公告链接导入、解析状态、需求确认与快照 | 已有真实接口；失败显示错误，不回退为演示数据 |
...
| 标书成果 | 分类、版本切换、预览、选区引用、单文件/打包下载交互及加载反馈 | 传统成果版本接口已接；新的真实文件产物、下载和渲染验收链路尚未全面接通 |
| Office 编辑 | 本地原文件编辑、选区引用、另存为新版本/覆盖当前版本 | 依赖可选本地 Office 服务；保存到本地卷，不是正式后端 |
| 评审与报价 | 评审任务和结果；历史价格查询与报价策略交互 | 按真实接口返回展示；不以 AI 猜价填补缺失数字 |
