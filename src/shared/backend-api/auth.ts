...
export const createAuthApi = (client: BackendApiClient) => ({
  register: (body: { email: string; password: string; enterprise_name: string }) =>
    client.request<TokenPair>('/auth/register', { method: 'POST', body, skipAuthRefresh: true }),
  login: (body: { email: string; password: string }) =>
    client.request<TokenPair>('/auth/login', { method: 'POST', body, skipAuthRefresh: true }),
  refresh: (refreshToken: string) =>
    client.request<TokenPair>('/auth/refresh', {
      method: 'POST', body: { refresh_token: refreshToken }, skipAuthRefresh: true,
    }),
