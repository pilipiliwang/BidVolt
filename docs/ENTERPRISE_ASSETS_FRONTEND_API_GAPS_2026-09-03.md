...
- 当前状态：`POST /files/upload` 仅能自动解包 ZIP；RAR/7Z 会被后端拒绝，`POST /files/archive` 也仅处理 ZIP。
- 前端临时行为：只声明 ZIP 可自动解包；RAR/7Z 提示用户先转换为 ZIP，不展示虚假成功状态。
- 后端需求：如需支持 RAR/7Z，应沿用 ZIP 的安全校验、解包溯源、重复文件和逐文件失败回执。

### 6. 历史标书成果智能提取企业资料

- 当前状态：后端没有“历史标书成果解析 → 抽取企业资料 → 分类入库”的专用接口或可执行任务；现有 `/enterprise/ingest` 只会按文件名重新归类已有资产。
- 前端临时行为：入口灰置为“历史标书成果智能提取（待后端支持）”。
- 最小契约：提供异步启动接口并返回 `task_id`；进度复用已有 `/tasks/{task_id}` 与 `/tasks/{task_id}/stream`；完成结果需实际落库，并返回资产、分类、证据位置、重复项及失败原因。
