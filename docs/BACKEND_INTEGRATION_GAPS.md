...
前端现已使用带 Bearer 和 401 刷新重放的 fetch 流式解析，并在断流或协议错误时回退轮询。SSE 仍只作为实时进度加速通道：收到终态后会再调用 `GET /tasks/{task_id}` 收敛到服务端最终状态。由于当前无心跳、事件 ID 和断点重放，弱网重连虽能获得最终状态，仍可能丢失中间进度或提示事件。
...
页面底部项目助手的“添加文件”属于本项目补充资料，但现有项目上传和文件列表均没有可持久化的用途字段。前端现已停止使用浏览器会话记录 `file_id` 进行伪分组；后端未返回用途时，文件只显示为“用途待分类”，不会被假装归入当前招标材料、补充资料或已完成标书。
