...
8. **AI 不能直接改写成果。** 当前“AI针对性修改”只把 Word 选区或报价表当前单元格/整行的纯文本上下文填入
   页面底部项目助手输入框并聚焦；不自动提交、不联网、不直接修改成果。项目助手接口未接入时发送按钮必须禁用。
   生产 AI 必须经过“生成建议、差异预览、用户确认应用”，并审计生成、应用、拒绝和撤销；未经确认不得修改
   工作副本或生成成果版本。
...
- “AI针对性修改”按钮只执行编辑器到项目助手的上下文交接：Word 使用当前选区，报价单使用当前单元格或整行，
  转为可编辑的纯文本提示并聚焦输入框。该动作不调用项目助手或建议接口；用户补充要求后仍需主动发送，未接入
  项目助手接口时发送按钮保持禁用。
