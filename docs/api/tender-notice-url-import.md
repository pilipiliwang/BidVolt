...
状态：前端与后端实现已在同名功能分支 `codex/tender-notice-url-import` 完成；部署前需先执行数据库迁移 `0015_tender_notice_import`。
...
成功响应：`202 Accepted`。当前实现会在请求内完成受控抓取并返回 `succeeded` 或 `failed`；后续可在不改变返回模型的前提下迁移为后台队列。
...
- 新增 `tender_notice_import` 持久化模型与列表/详情审计入口。
- `FileObject.source_metadata` 保存来源网址、最终网址、标题、抓取时间和内容哈希。
- HTML 正文保存为当前项目可解析文本，同源公开附件分别进入项目文件处理链。
- 对每次 DNS 解析及重定向做 SSRF 校验，并固定连接到校验后的 IP，防止 DNS rebinding。
- ZIP/RAR/7Z 在解包前校验路径、链接、条目数量、总体大小及目录深度；缺少系统解包工具时返回稳定错误码。
