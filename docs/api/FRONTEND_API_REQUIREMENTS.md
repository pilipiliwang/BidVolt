...
| 成果编辑器 | 已有选区直接填入；无选区进入预览拖选；单元格/整行填入项目助手 | 无需接口 | 浏览器内交互 | - |
...
“AI针对性修改”按钮也是浏览器内交互。Word 已有非空选区时点击会直接把选区纯文本写入项目助手输入框并聚焦；
没有选区时点击只进入预览选取模式，用户拖选文字并松开指针后自动提取纯文本、退出模式、写入并聚焦输入框，
选取过程中按 `Esc` 可取消且不填入上下文。报价表使用当前单元格或整行。该动作不自动提交、不发起网络请求、
不直接修改正文或表格，也不调用 FE-EDITOR-004～006；项目助手接口未接入时发送按钮必须禁用。
...
Word 已有选区时点击“AI针对性修改”直接填入并聚焦输入框；无选区时先进入预览选取模式，拖选并松开后才填入，
`Esc` 可取消。报价表带入当前单元格/整行。以上交互不能自动调用以下接口，用户可以继续补充、删改上下文；接口
接入后也只有用户主动点击发送才允许发起请求。
...
- Word 已有选区时直接把纯文本写入项目助手输入框；无选区时进入预览选取模式，拖选松开后自动填入，`Esc`
  取消；报价表带入当前单元格/整行。以上动作只聚焦输入框，不自动发送。
...
“AI针对性修改”只完成编辑器上下文到项目助手输入框的交接；Word 无选区时的预览拖选、松开填入和 `Esc` 取消
同样不调用 FE-EDITOR-004～006。接入生产 AI 后必须使用该接口组并执行差异预览、用户确认和审计。账户菜单本身
由前端实现，但其中的“退出登录”必须调用
...
- Word 已有非空选区时点击“AI针对性修改”直接写入并聚焦项目助手输入框；无选区时点击只进入预览选取模式，
  拖选文字并松开后自动填入；按 `Esc` 取消时不填入、不修改正文。
- 报价表“AI针对性修改”带入当前单元格/整行。两类操作都不自动发送、不产生网络请求、不直接修改正文/表格；
  项目助手接口未接入时发送按钮禁用。
