...
> 状态：前端已以后端运行时 OpenAPI 和真实 API 响应为准进行联调。以下未在后端运行时契约中提供的接口仍只是评审草案，不视为已上线能力。
...
当前“AI针对性修改”不生成建议。Word 已有非空选区时，点击后直接把选区纯文本填入页面底部项目助手输入框；
没有选区时，点击后进入预览选取模式，用户拖选文字并松开指针后自动提取纯文本、退出模式、填入并聚焦输入框，
选取过程中按 `Esc` 可取消且不填入上下文。报价表使用当前单元格/整行。以上动作只供用户继续补充要求，不自动
提交、不发起网络请求，也不直接修改正文或表格；项目助手接口未接入时发送按钮必须禁用。
...
- Word 已有选区时点击“AI针对性修改”会直接把选区纯文本填入并聚焦项目助手输入框；无选区时点击只进入预览
  选取模式，拖选并松开后才自动填入，按 `Esc` 取消时不填入、不修改正文。
- 报价表点击“AI针对性修改”会带入当前单元格/整行纯文本。两类入口都不自动发送、不发起网络请求、不修改
  正文/表格；项目助手接口未接入时发送按钮保持禁用。
