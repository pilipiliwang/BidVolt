...
| `codex/data-contracts` | 真实 API client、DTO 适配、OpenAPI、公开事件白名单 | contract test、OpenAPI 解析 |
...
| `codex/web-mvp` | 汇总路由、项目级状态隔离、真实 API 数据/失败态、CI 与全量验收 | 全量门禁 |
...
- [x] 完成 OpenAPI 评审稿、真实 API client、DTO 适配和 Contract 测试流程。
...
2. `src/shared/backend-api` 的请求封装、DTO 与适配器
3. Contract 与领域隔离测试
